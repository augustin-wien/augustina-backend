...
func (_q *PaymentQuery) QueryParent() *PaymentQuery {
...
func (_q *PaymentQuery) QueryChildren() *PaymentQuery {
	query := (&PaymentClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *PaymentQuery) WithParent(opts ...func(*PaymentQuery)) *PaymentQuery {
	query := (&PaymentClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *PaymentQuery) WithChildren(opts ...func(*PaymentQuery)) *PaymentQuery {
	query := (&PaymentClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *PaymentQuery) loadParent(ctx context.Context, query *PaymentQuery, nodes []*Payment, init func(*Payment), assign func(*Payment, *Payment)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Payment)
	for i := range nodes {
...
func (_q *PaymentQuery) loadChildren(ctx context.Context, query *PaymentQuery, nodes []*Payment, init func(*Payment), assign func(*Payment, *Payment)) error {
