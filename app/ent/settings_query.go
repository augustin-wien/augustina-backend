...
// QueryMainItem chains the current query on the "MainItem" edge.
func (sq *SettingsQuery) QueryMainItem() *ItemQuery {
...
func (sq *SettingsQuery) Clone() *SettingsQuery {
	...
	return &SettingsQuery{
		...
		withMainItem: sq.withMainItem.Clone(),
...
// WithMainItem tells the query-builder to eager-load the nodes that are connected to
// the "MainItem" edge. The optional arguments are used to configure the query builder of the edge.
func (sq *SettingsQuery) WithMainItem(opts ...func(*ItemQuery)) *SettingsQuery {
	query := (&ItemClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	sq.withMainItem = query
	return sq
}
...
func (sq *SettingsQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Settings, error) {
	...
	if query := sq.withMainItem; query != nil {
		if err := sq.loadMainItem(ctx, query, nodes, nil,
			func(n *Settings, e *Item) { n.Edges.MainItem = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *SettingsQuery) loadMainItem(ctx context.Context, query *ItemQuery, nodes []*Settings, init func(*Settings), assign func(*Settings, *Item)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Settings)
	for i := range nodes {
		if nodes[i].mainitem == nil {
			continue
		}
		fk := *nodes[i].mainitem
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "mainitem" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
