...
func (lq *LocationQuery) QueryVendor() *VendorQuery {
	query := (&VendorClient{config: lq.config}).Query()
...
func (lq *LocationQuery) WithVendor(opts ...func(*VendorQuery)) *LocationQuery {
	query := (&VendorClient{config: lq.config}).Query()
...
func (lq *LocationQuery) loadVendor(ctx context.Context, query *VendorQuery, nodes []*Location, init func(*Location), assign func(*Location, *Vendor)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Location)
	for i := range nodes {
		if nodes[i].vendor_locations == nil {
			continue
		}
		fk := *nodes[i].vendor_locations
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "vendor_locations" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
