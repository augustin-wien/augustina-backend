...
func (oq *OrderQuery) QueryEntries() *OrderEntryQuery {
	query := (&OrderEntryClient{config: oq.config}).Query()
...
func (oq *OrderQuery) Clone() *OrderQuery {
	...
	return &OrderQuery{
		...
		withEntries:  oq.withEntries.Clone(),
...
func (oq *OrderQuery) WithEntries(opts ...func(*OrderEntryQuery)) *OrderQuery {
	query := (&OrderEntryClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	oq.withEntries = query
	return oq
}
...
func (oq *OrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Order, error) {
	...
	if query := oq.withEntries; query != nil {
		if err := oq.loadEntries(ctx, query, nodes,
...
func (oq *OrderQuery) loadEntries(ctx context.Context, query *OrderEntryQuery, nodes []*Order, init func(*Order), assign func(*Order, *OrderEntry)) error {
