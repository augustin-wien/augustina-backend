...
func (cq *CommentQuery) QueryVendor() *VendorQuery {
	query := (&VendorClient{config: cq.config}).Query()
...
func (cq *CommentQuery) WithVendor(opts ...func(*VendorQuery)) *CommentQuery {
	query := (&VendorClient{config: cq.config}).Query()
...
func (cq *CommentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Comment, error) {
	...
	if query := cq.withVendor; query != nil {
		if err := cq.loadVendor(ctx, query, nodes, nil,
			func(n *Comment, e *Vendor) { n.Edges.Vendor = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CommentQuery) loadVendor(ctx context.Context, query *VendorQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *Vendor)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Comment)
	for i := range nodes {
		if nodes[i].vendor_comments == nil {
			continue
		}
		fk := *nodes[i].vendor_comments
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "vendor_comments" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
