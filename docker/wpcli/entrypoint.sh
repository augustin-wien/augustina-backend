...
export WP_API_KEY=$(wp --allow-root user application-password create api_user augustin --porcelain --allow-root || echo "")
echo "WP_API_KEY=$WP_API_KEY"
(sed -i '3s/.*/WP_API_KEY='$WP_API_KEY'/' /wpcli/parser/.env.parser || true)
(cat /wpcli/parser/.env.parser || true)
...
wp --allow-root config set OIDC_ENDPOINT_JWKS_URL ${KEYCLOAK_URL}realms/augustin/protocol/openid-connect/certs
wp --allow-root config set OIDC_ISSUER ${KEYCLOAK_URL}realms/augustin
...
wp --allow-root post create --post_title="magazin-1" --post_status=publish --post_type="ausgabe" /wpcli/demo_content/papers/paper-1/magazin-1.txt
wp --allow-root post create --post_title="test2" --post_status=publish --post_type="ausgabe" /wpcli/demo_content/papers/paper-2/magazin-2.txt
wp --allow-root post create --post_title="test3" --post_status=publish --post_type="ausgabe" /wpcli/demo_content/papers/paper-3/magazin-3.txt
...
(sed -i '3s/.*/WP_API_KEY='$(wp --allow-root user application-password create api_user augustin --porcelain --allow-root)'/' /wpcli/parser/.env.parser || true)
